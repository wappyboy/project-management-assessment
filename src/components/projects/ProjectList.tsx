...
export function ProjectList({ projects, isLoading }: ProjectListProps) {
  ...
  function handleSearch(value: string) {
    setSearch(value);
    setPage(1); // reset to page 1 on every search change
  }
    ...
    <div className="space-y-4">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-xl border border-zinc-200 bg-white py-2 pl-9 pr-4 text-sm text-zinc-800 placeholder:text-zinc-400 shadow-sm outline-none focus:border-zinc-400 focus:ring-2 focus:ring-zinc-100 transition"
        />
      </div>
          ...
          {totalPages > 1 ? (
            <div className="flex flex-col gap-3 border-t border-zinc-100 pt-4 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-center text-xs text-zinc-500 sm:text-left">
                Page {safePage} of {totalPages} · {filteredProjects.length} project{filteredProjects.length !== 1 ? "s" : ""}
